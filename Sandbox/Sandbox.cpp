...
namespace Voxel {
    Sandbox::Sandbox() : Engine(), _targetPosition(std::make_shared<glm::vec3>(0.0f)), _targetAngle(0.0f), _mainAngle(0.0f) {}
    ...
    void Sandbox::onUpdate() {
        ...
        auto cubeMesh = this->_cubeMesh.lock();
        auto material = this->_material.lock();
        if (cubeMesh && material) {
            ...
            getServiceLocator().getManager<RenderManager>().submitMesh(cubeMesh, material, mainModel);
            ...
            getServiceLocator().getManager<RenderManager>().submitMesh(cubeMesh, material, targetModel);
        }
    }
